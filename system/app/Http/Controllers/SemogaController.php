...
class SemogaController extends Controller
{
    public function getIndex()
    {
        ...
        {
            return view('semoga.read-sukses')
                ->with('kegiatans',$kegiatans)
                ->with('id',Session::get('sukses'));
        }
        return view('semoga.read')->with('kegiatans',$kegiatans);
    }
    public function getListdonor()
    {
        ...
        $donors = Donor::where('nama','like','%'.Session::get('proses_search').'%')->paginate(12);
        return view('semoga.listDonor')
            ->with('donors',$donors);
    }
    public function getListantrian()
    {
        ...
        else
        {
            ...
            if(count($details) > 0)
            {
                return view('semoga.listAntrian')->with('details',$details);
            }
            else
            {
    ...
    public function getTambahdonor()
    {
        ...
        return view('semoga.tambahDonor')->with([
            'golDarahs'=>$golDarahs,
...
//            if (Input::get('donor_terakhir') != "") $data += Input::get('donor_terakhir');
            ...
            return Redirect::to('semoga')->with('sukses',$donor->id);
//        }
    }
    ...
    public function getYes()
    {
        ...
        return view('semoga.yes')->with('kegiatans',$kegiatans);
    }
    public function getBisa()
    {
        $antrian = DetailKegiatan::where('kegiatan_id',Session::get('proses_kegiatan'))->max('antrian');

        $detail = DetailKegiatan::create([
            'donor_id' => Input::get('id'),
            'kegiatan_id' => Session::get('proses_kegiatan'),
            'antrian' => ++$antrian,
            'panggil' => 'Belum',
            'terima' => 'Tidak',
        ]);
        return view('semoga.bisaprint')->with('id',Input::get('id'));
    }
}
