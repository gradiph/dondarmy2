...
-->
        ...
        <div class="col-lg-6 form-group">
            <div class="col-lg-8 form-group">
                <div class="input-group">
                    <input class="form-control" id="search" value="{{ Session::get('proses_search') }}"
                           onkeyup="if ((event.keyCode >= 48 && event.keyCode <= 90) || event.keyCode == 13 || event.keyCode == 8 || event.keyCode == 46) ajaxLoad('{{url('semoga/listdonor')}}?ok=1&search='+this.value,'donor')"
                           placeholder="Cari Nama ..."
                           type="text"
                           autofocus>
                    <div class="input-group-btn">
                        <button type="button" class="btn btn-default"
                                onclick="ajaxLoad('{{url('semoga/listdonor')}}?ok=1&search='+$('#search').val(),'donor')"><i
            ...
            <div class="col-lg-4">
                <a href="{{ url('semoga/tambahdonor') }}" class="btn btn-block btn-danger"><i class="glyphicon glyphicon-plus"></i> Tambah Donor</a>
            </div>
...
$(document).ready(function(){
    ajaxLoad("{{ url('semoga/listdonor') }}",'donor');
    ajaxLoad("{{ url('semoga/listantrian') }}",'antrian');
    $(".alert").click(function(){
        $(this).hide('slow');
    });
    $("#proses_kegiatan").change(function(){
        ajaxLoad("{{ url('semoga/listantrian?id=') }}" + $(this).val(),'antrian');
    });
    var donor_id = "{{ $id }}";
